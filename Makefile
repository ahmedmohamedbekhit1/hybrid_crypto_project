...
build-liboqs-local:
	@$(PYTHON) -c "import os, pathlib, shutil, subprocess; root = pathlib.Path('.').resolve(); install_prefix = root / '_oqs_install'; dll = install_prefix / 'bin' / 'oqs.dll'; dll.exists() and print('liboqs already built; skipping rebuild.') or (lambda: (env := {**os.environ, 'PATH': str(root / '.venv' / 'Scripts') + ';' + os.environ.get('PATH', '')}, cmake := shutil.which('cmake', path=env['PATH']) or str((root / '.venv' / 'Scripts' / 'cmake.exe').resolve()), subprocess.check_call([cmake, '-S', str(root / 'liboqs'), '-B', str(root / '_liboqs_build'), '-G', 'Visual Studio 17 2022', '-A', 'x64', '-DBUILD_SHARED_LIBS=ON', '-DOQS_BUILD_ONLY_LIB=ON', f'-DCMAKE_INSTALL_PREFIX={install_prefix}'], env=env), subprocess.check_call([cmake, '--build', str(root / '_liboqs_build'), '--config', 'Release'], env=env), subprocess.check_call([cmake, '--install', str(root / '_liboqs_build'), '--config', 'Release'], env=env), print('liboqs build/install completed.')))()"
