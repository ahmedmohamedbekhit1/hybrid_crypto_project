...
install-oqs-local:
	@$(PYTHON) -c "from pathlib import Path; import subprocess, sys; target = Path('$(OQS_PY_PATH)'); pip = Path('.venv/Scripts/pip.exe'); ok = target.exists() and ((target/'pyproject.toml').exists() or (target/'setup.py').exists()); ok or sys.exit('Local oqs bindings path invalid. Use: make install-oqs-local OQS_PY_PATH=PATH'); subprocess.check_call([str(pip), 'install', str(target)])"
